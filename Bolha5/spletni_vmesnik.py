...
def nastavi_sporocilo(sporocilo, piskotek='sporocilo'):
    bottle.response.set_cookie(piskotek, sporocilo, secret=SKRIVNOST, path='/', samesite='Lax')
...
def prijavi_uporabnika(uporabnik, piskotek=None):
    ...
    bottle.response.set_cookie('uporabnik', str(uporabnik.id), secret=SKRIVNOST, path='/', samesite='Lax')
    if piskotek:
        izbrisi_piskotek(piskotek)
    bottle.redirect('/')
...
def index(uporabnik):
    ...
    stran = int(bottle.request.query.stran or 1)
    na_stran = 30

    oglasi = Oglas.poisci_filtrirane(
        iskanje=iskanje or None,
        kategorija_id=int(kategorija_id) if kategorija_id else None,
        tip=tip or None
    )

    skupaj = len(oglasi)
    stevilo_strani = (skupaj + na_stran - 1) // na_stran
    oglasi_stran = oglasi[(stran - 1) * na_stran : stran * na_stran]

    kategorije = Kategorija.poisci_vse()
    return dict(oglasi=oglasi_stran, iskanje=iskanje, kategorija_id=kategorija_id,
                tip=tip, kategorije=kategorije, stran=stran,
                stevilo_strani=stevilo_strani, skupaj=skupaj)
...
def moji_oglasi(uporabnik):
    oglasi = Oglas.poisci_po_uporabniku(uporabnik.id)
...
bottle.BaseTemplate.defaults['Oglas'] = Oglas
